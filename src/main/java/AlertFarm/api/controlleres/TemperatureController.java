...
public class TemperatureController {
    ...
    @RequestMapping(path = "/temperatures/{correo}", method = RequestMethod.GET )
    @ResponseBody
    public ResponseEntity<List<Temperature>> getTemperatures(@PathVariable("correo") String id){

        System.out.print("email "+id+" ");

        try {
            return ResponseEntity.ok().body(temperatureService.getTemperatureListId(id));
        } catch (ServicesException ex) {
            Logger.getLogger(HumidityController.class.getName()).log(Level.SEVERE, null, ex);
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
